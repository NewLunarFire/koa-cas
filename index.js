    ...
    return {
        ...
        'callback': async function(ctx, next) {
            if(ctx.request.query.ticket) {
                const uri = baseUrl + '/serviceValidate?ticket=' + ctx.request.query.ticket + '&service=' + encodeURIComponent(options.service);
                ...
                if(casResponse["cas:serviceResponse"] && casResponse["cas:serviceResponse"]["cas:authenticationSuccess"]) {
                    ...
                    Object
                        .keys(attributes)
                        .forEach((key) => ctx.cas.user[key.substr(4)] = attributes[key]);
